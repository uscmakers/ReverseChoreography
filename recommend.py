...
def main():
    ...
    #TODO change preprocess args to give a file generated by motion capture of current user
    [x_data, y_data] = preprocess("data/10_lite_spots_kaytranada.csv")
    #Looks like parser takes a considerable amount of time
...
#TODO add through data set 11 or 12, 12 is more interesting data, also add 14, use 13 and maybe 12 as test data
...
def generate_regression_features(extracted_features):
    #Load in saved model
    filename = 'model.sav'
    model = pickle.load(open(filename, 'rb'))
...
def main():
    ...
    predicted_audio_features = generate_regression_features(extracted_features)
    features = np.append(predicted_audio_features[0][0:len(predicted_audio_features[0])-1], tempo)
    show_feature_based_recommendations_for_song(features)
    
    predicted_song_number = generate_classification_preduction(extracted_features)


if __name__ == '__main__':
    main()
>>>>>>> a65a4199ab14c23ad3358fd59e3740d12b4aeeca
